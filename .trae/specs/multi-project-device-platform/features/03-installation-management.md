...
作为安装管理员，我需要创建安装任务，选择要安装的设备，设置计划时间并分配执行人员。安装人员接到任务后到达现场，确认安装位置，执行安装操作。安装过程中我需要填写检查清单，确保所有必检项都已完成，上传安装过程照片记录现场情况。如果遇到问题可以记录问题描述并反馈。安装完成后提交验收申请，验收人员进行现场验收，通过电子签名确认验收结果。任务完成后设备状态自动更新。
