...
作为设备管理员，我需要创建设备台账，选择设备所属专题和型号，填写设备编码、IMEI号等基本信息，设置安装位置（区县、街道、详细地址）并在地图上标记经纬度，上传设备照片和现场照片。设备入库后可进行调试，调试完成后设备状态变为在线。我可以为设备制定巡检计划，系统自动生成巡检任务分配给巡检人员。巡检人员到达现场后按模板填写巡检记录、上传巡检照片。当设备出现问题时，我可以创建维护工单，分配给维护公司处理，工单完成后进行验收。现场人员发现故障时可上报故障，填写故障类型、级别和描述，上传现场照片，处理人分析原因并记录解决方案。
...
1. 选择设备所属专题
2. 选择设备型号（关联设备类型和厂商）
3. 填写设备基本信息（编码、第三方编码、IMEI等）
4. 填写位置信息（区县、街道、详细地址、经纬度）
5. 设置点位名称，是否在窨井内
6. 上传设备照片、现场照片
7. 填写现场情况说明
8. 提交入库
