...
业务配置模块负责管理平台业务层面的各类配置，包括设备类型、设备厂商、设备型号、专题、维护公司、备件库等。**业务配置按项目隔离**，每个项目有独立的配置数据，确保不同项目之间的业务配置互不影响。

## 2. 用户故事

作为业务配置管理员，我需要为项目配置设备类型，创建树形结构的设备类型层级并设置唯一编码。我可以管理设备厂商信息，创建设备型号并关联到对应的类型和厂商。专题管理方面，我创建专题后配置该专题可管理的设备类型，再为用户分配专题访问权限，用户登录后只能看到授权的专题数据。维护公司管理包括创建公司并配置可维护的设备型号。备件库管理涵盖备件分类、库存管理和备件申请审批流程。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/config/users/{userId}/topics | 获取用户专题权限列表 |
| POST | /api/config/users/{userId}/topics | 添加用户专题权限 |
| DELETE | /api/config/users/{userId}/topics/{topicId} | 移除用户专题权限 |
